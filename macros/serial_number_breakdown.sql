{% macro is_valid_serial_number(serial_number) %}
    CASE
        WHEN LENGTH({{ serial_number }}) = 13 AND {{ serial_number }} ~ '^\d+$' AND CAST(SUBSTRING({{ serial_number }}, 3, 2) AS INT) <= 12 THEN 'yes'
        WHEN LENGTH({{ serial_number }}) = 8 AND {{ serial_number }} ~ '^\d+$' AND CAST(SUBSTRING({{ serial_number }}, 3, 2) AS INT) <= 12 THEN 'no'
        ELSE 'no'
    END
{% endmacro %}
...
{% macro serial_build_date(serial_number) %}
    CASE
        WHEN {{ is_valid_serial_number(serial_number) }} = 'yes' THEN
